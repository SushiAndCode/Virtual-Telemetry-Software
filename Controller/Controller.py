...
class Controller():
    ...
    def setUpUI(self):
        ...
        with dpg.window(label="Telemetry Viewer", width=1000, height=600, tag="main_window"):
            dpg.add_menu_item(label="Load File", callback=lambda: dpg.show_item("file_dialog_id"))
        
    def csvRetriever(self,sender, app_data):
        # Object laps
        self.Laps = lp.Laps(pd.read_csv(app_data['file_path_name']))
        # Df from the object
        self.df = self.Laps.df
        # laps of the telemetry from object Laps
            ...
            with dpg.group(horizontal=True):
                ...
                with dpg.group():
                    with dpg.plot(label="Telemetry",height=400, width=-1, tag = "Main Plot"):
                        ...
                        dpg.set_axis_ticks("main_x_axis", self.Laps.getCustomTimeUnits(5000))
                    ...
                    with dpg.plot(label = "Track",height=400, width=-1, tag="track_plot"):
                        ...
                        with dpg.plot_axis(dpg.mvYAxis, label="Y position", tag="positionY"):
                            dpg.add_line_series(self.df["PositionX"].tolist(), self.df["PositionZ"].tolist(), parent="positionY", label="Plotted Track")
